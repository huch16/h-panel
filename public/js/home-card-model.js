/* =========================================================================
 * home-card-model.js
 * 浏览器版 buildCardTemplateConfig + buildCardViewModel
 * 复刻 functions/lib/card-model.js（兼容 Node 与浏览器）
 * ========================================================================= */
(function () {
  'use strict';
  ...
  function buildSearchText(site, normalizedUrl) {
    return [
      site?.name, site?.url, normalizedUrl || '',
  ...
  function getDeviceSetting(settings, device, key, fallback = '') {
    ...
    if (device === 'mobile' && settings[mobileKey] !== undefined) return settings[mobileKey];
    return settings[key] ?? fallback;
  }

  function getDeviceSettingOrDefault(settings, device, key, fallback = '') {
    const v = getDeviceSetting(settings, device, key, fallback);
    return String(v ?? '').trim() === '' ? fallback : v;
  }

  function buildCardTemplateConfig(settings = {}, device = 'desktop') {
    const isMobile = device === 'mobile';
    const cardStyle = getDeviceSetting(settings, device, 'layout_card_style', isMobile ? 'style2' : 'style1') || (isMobile ? 'style2' : 'style1');
    const isNav3 = cardStyle === 'style3';
    const hideDesc = isNav3 || getDeviceSetting(settings, device, 'layout_hide_desc', isMobile) === true;
    const hideLinks = isNav3 || getDeviceSetting(settings, device, 'layout_hide_links', isMobile) === true;
    const hideCategory = isNav3 || getDeviceSetting(settings, device, 'layout_hide_category', false) === true;
    ...
    return {
      device, hideDesc, hideLinks, hideCategory, enableFrostedGlass,
      cardStyle, cardAnimation, gridCols, hideCopyText,
      titleSize, titleColor, titleFont,
      descSize, descColor, descFont,
      cardRadius, frostedGlassIntensity,
      ...
      cardStyleClass: cardStyle === 'style2' ? 'style-2' : (isNav3 ? 'style-3' : ''),
  ...
  function buildCardViewModel(site) {
  ...
  window.HomeCardModel = { buildCardTemplateConfig, buildCardViewModel, escapeHTML, sanitizeUrl };
})();